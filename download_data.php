<?php
	...
	if($_GET["action"]=="getData"){
		echo "<table class='table table-hover'>";
            ...
            echo "</tr>";
            $i=1;
            $sql="select news_id , day_in , title , count_view from tb_download order by news_id desc";
            $sql=$sql . " LIMIT 10 ";
    ...
    if($_GET["action"]=="getForm"){
        ...
        if($id !=""){
            $sql="select * from tb_download where news_id = $id";
            $result=mysql_db_query($database,$sql);
        ...
        echo "</div>";
        echo " <form action='download_data.php?action=saveData' method='post' target='upload_target' onsubmit='clickupload();' >";
    ...
    if($_GET["action"]=="getDocList"){
        ...
        echo "<table class='table table-hover'>";
            ...
            echo "</tr>";
            $i=1;
            $sql="select autoid , file_name , file_value from tb_download_item";
            $sql=$sql . " where news_id = $id order by autoid";
            $result=mysql_db_query($database,$sql);
            while($row=mysql_fetch_array($result)){
                echo "<tr>";
                    echo "<td>$i</td>";
    ...
    if($_GET["action"]=="deleteDoc"){
        $id=$_POST["id"];
        $sql="delete from tb_download_item where autoid = $id";
    ...
    if($_GET["action"]=="getFormUpload"){
        ...
        echo "<div class='box-body'>";
            ...
            echo "</div>";
            echo " <form id='frmDoc' action='download_data.php?action=insertDoc' enctype='multipart/form-data' method='post' target='upload_target' onsubmit='clickuploadDoc();' >";
                echo "<div class='form-group'>";
            ...
            if($_FILES["fileField"]["error"]==4){
                ...
                //}else{
                    ...
                    if(@move_uploaded_file($tempfile,$target_path)){
                        $sql="insert into tb_download_item(news_id , file_name , file_value )";
    ...
    if($_GET["action"]=="getView"){
		...
		$sql="update tb_download set count_view=count_view+1 where news_id=$id";
		$result=mysql_db_query($database,$sql);
        ...
        echo "<div class='box-body'>";
            ...
            echo "<p>$detail</p>";
            $sql="select * from tb_download_item where news_id = $id order by autoid";
            $result=mysql_db_query($database,$sql);
    ...
    if($_GET["action"]=="delete"){
        ...
        $sql="delete from tb_download where news_id = $id";
        $result=mysql_db_query($database,$sql);
        exit();
    }
    ...
    if($_GET["action"]=="saveData"){
        ...
        if($txtTitle ==""){
            ...
            if($id !=""){
                $sql="update tb_download set day_in='$day_in' , title='$txtTitle' ";
                $sql=$sql . " , detail='$txtDetail' where news_id=$id ";
            }else{
                $sql="insert into tb_download(day_in , title , detail , count_view ) ";
