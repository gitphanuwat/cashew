<?php
	...
	if($_GET["action"]=="memberNew"){
		echo "MEMBER";
	}
	if($_GET["action"]=="groupNew"){
		echo "GROUP";
	}
	if($_GET["action"]=="typeNew"){
		echo "TRADE";
	}
	...
	if($_GET["action"]=="loaddata"){
		$sql="select tbtype.icon, tbbase.namebase, tbtype.nametype, tbbase.idbase from tbbase, tbtype where tbbase.idtype=tbtype.idtype and iduser = $iduser order by tbbase.idbase ASC";
		$result=mysql_db_query($database,$sql);
            ...
            <th width="70">Tools</th>
	   		...
	   		while($row=mysql_fetch_array($result)){
				echo "<tr>";
					...
					echo "<td>$row[1]</td>";
					echo "<td>$row[2]</td>";
					echo "<td><a href='#$row[3]' title='แก้ไขข้อมูล' class='updateItem'><img src='img/edit.png'></a> &nbsp;&nbsp;";
