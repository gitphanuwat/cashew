...
<body class="homepage">
    <div style="font-size: 12px;" id="dialog" title="ข้อมูลดาวน์โหลด"> 
...
?>
        ...
        <script type="text/javascript">
			$(document).ready(function(){
                ...
                $("#boxView").hide();
                $("#showData").load("admininform_data.php?action=getData",function(){
                    $("#loadBar").fadeOut();
                });

                $("#butNew").click(function(){
                    $("#dialog-from").load("admininform_data.php?action=getForm",function(){
            ...
            function stopUpload(success , error ,actionPage , id ){
                if(actionPage=="news"){
                    ...
                    if(success ==1){
                        $("#showDoc").load("admininform_data.php?action=getDocList&id="+id);
                    }else{
                        if(error==1){
            ...
            $(document).on('click',".delItemsDoc",function(){
                ...
                if(confirm("ท่านต้องการลบรายการนี้หรือไม่ ?")){
                    $.post("admininform_data.php?action=deleteDoc",{id:yid[0]},function(){
                        $("#showDoc").load("admininform_data.php?action=getDocList&id="+yid[1]);
                    });
                }
            });
            ...
            $(document).on('click',".editItem",function(){
                var id = $(this).attr("href");
                id = id.replace("#","");
                $("#dialog-from").load("admininform_data.php?action=getForm&id="+id,function(){
                    $("#dialog").dialog( "option", "width", 450 );
                    $( "#dialog" ).dialog( "open" );
                });
            });
            ...
            $(document).on("click",".delItem",function(){
                ...
                if(confirm("ท่านต้องการลบรายการนี้หรือไม่ ?")){
                    $("#loadBar").fadeIn();
                    $.post("admininform_data.php?action=delete",{id:id},function(){
                        $("#showData").load("admininform_data.php?action=getData",function(){
                            $("#loadBar").fadeOut();
                        });
                    });
            ...
            $(document).on("click",".uploadItem",function(){
                var id = $(this).attr("href");
                id = id.replace("#","");
                $("#dialog-from").load("admininform_data.php?action=getFormUpload&id="+id,function(){
                    $("#showDoc").load("admininform_data.php?action=getDocList&id="+id);
                    $("#dialog").dialog( "option", "width", 450 );
                    $( "#dialog" ).dialog( "open" );
                });
            ...
            $(document).on("click",".viewItem",function(){
                var id = $(this).attr("href");
                id = id.replace("#","");
                $("#boxView").load("admininform_data.php?action=getView&id="+id,function(){
                    $("#boxView").show();
                });
             });
