...
<body class="homepage">
    <div style="font-size: 12px;" id="dialog" title="ข้อมูลเฟสบุ๊ค"> 
                    	...
                    	<div class="col-xs-12">
                        	<div class="box">
                            	<div class="box-header">
                                    <h3 class="box-title">ข้อมูลข่าว&กิจกรรม จากเฟสบุ๊ค</h3>
                                </div><!-- /.box-header -->
        ...
        <script type="text/javascript">
			$(document).ready(function(){
                ...
                $("#butSearch_edit").click(function(){
                    ...
                    $("#loadInfo").fadeIn();
                    $("#boxRegistered").load("adminDataNews.php?action=getRegistered&search=" + valSearch ,function(){
                        $("#loadInfo").fadeOut();
                    });
                });
				
                //กำลังลงทะเบียน
				$("#boxRegistered").load("adminDataNews.php?action=getRegistered" ,function(){
					$("#loadRegistered").fadeOut();
				});

                $("#butNew").click(function(){
                    $("#dialog-from").load("adminDataNewsForm.php?action=getFormNews",function(){
			...
			$(document).on('click','.delItemRegistered', function() { 
				...
				if(confirm("ท่านต้องการลบรายการนี้หรือไม่ ?")){
					...
					$.post("adminDataNews.php?action=delUser",{id:id},function(){
						$("#boxRegistered").load("adminDataNews.php?action=getRegistered" ,function(){
							$("#loadRegistered").fadeOut();
						});
					});
            ...
            $(document).on('click','.viewItemRegistered',function(){
                var id=$(this).attr("href");
                id=id.replace("#","");
                $("#dialog-from").load("adminDataNews.php?action=getDetail&id="+id,function(){
                    $("#dialog").dialog( "option", "width", 400 );
                    $( "#dialog" ).dialog( "open" );
                });
            });
            ...
            $(document).on('click','.editItemRegistered',function(){
                var id=$(this).attr("href");
                id=id.replace("#","");
                $("#dialog-from").load("adminDataNewsForm.php?action=getFormNews&id="+id,function(){
                    $("#dialog").dialog( "option", "width", 400 );
                    $( "#dialog" ).dialog( "open" );
                    $("#loadDialog").fadeOut();
                });
