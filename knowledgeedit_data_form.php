<?php
	...
	if($_GET["action"]=="getgroup"){
		$id=$_GET["id"];
		if($id !=""){
			$sql="select * from tbknowgroup where idgroup = $id";
		...
		echo " <form action='knowledgeedit_data_form.php?action=updategroup' method='post' enctype='multipart/form-data' target='upload_target' onsubmit='clickSavegroup();' >";
            	echo "<div class='form-group'>";
            		echo "<label >ชื่อกลุ่ม<font color=\"red\">*</font></label>";
            		echo "<input type='text' class='form-control' name='namegroup' value='$namegroup'>";
            		echo "<label >รายละเอียด</label><br>";
					echo "<textarea name='detail' cols='57' id='detail'>$detail</textarea>";
	...
	if($_GET["action"]=="getFromMember"){
		...
		echo " <form action='knowledgeedit_data_form.php?action=insert' method='post' target='upload_target' onsubmit='clickSave();' >";
